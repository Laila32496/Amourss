jQuery(function($) {
    var styleSwitcher = {
        ...
        initialize : function() {
            ...
            var $templateURL = charity.template_url;

            // Style Switcher CSS
            //$("head").append($('<link rel="stylesheet">').attr("href", $templateURL+"assets/js/style-switcher/assets/css/font-awesome.css"));
            //$("head").append($('<link rel="stylesheet">').attr("href", "assets/js/style-switcher/assets/css/theme_panel.css"));
            $("head").append($('<link rel="stylesheet">').attr("href", "../../../../../code.jquery.com/ui/1.11.1/themes/smoothness/jquery-ui.css"));
            $("head").append($('<link rel="stylesheet/less">').attr("href", $templateURL + "/assets/switcher/assets/css/skin.less"));
            $("head").append($('<script />').attr("src", "../../../../../code.jquery.com/ui/1.11.1/jquery-ui.js"));

            $.getScript($templateURL + "/assets/switcher/assets/js/less.js", function(data, textStatus, jqxhr) {
                //$.getScript($templateURL+'/assets/switcher/assets/js/jquery.easing.js', function(data, textStatus, jqxhr) {
                //???$("body").append('<div id="theme_panel"></div> <div id="all-headers" />');
                //$("#theme_panel").load($templateURL+"/assets/switcher/switcher.html", function(response, status, xhr) {
        ...
        build : function() {
            //console.log("CODE ERROR: 7");

            var $this = this;

            this.container = $("#theme_panel");

            /*global color setting code starts here*/
            // Colors Skins
            var colors = [
                /*{
            ...
            $('body').on('click', '.swatches a', function(e) { 
                e.preventDefault();
                $this.setColor($(this).attr("data-color-hex"));
            });
           // console.log("CODE ERROR: 8");

            var globalColor = charity.color;
            var colorContainer = this.container.find('.swatches li');

            if(globalColor !='')
            { 
                for (var i = 0; i < colorContainer.length; i++) {

                    if (globalColor == $(colorContainer[i]).find('.color-fill').attr('data-color-hex')) {
					
                        $this.setColor(globalColor);
                        //***return false; //CODE ERROR: why is return

                    } else {
					
                        if (i == colorContainer.length - 1) {
                            //alert(1111)
                            var color = $("<li />").append($("<a class='color-fill' />").css("background-color", globalColor).attr({
                                "data-color-hex" : globalColor,
                                "href" : "#"
                            }).append($('<i/>').addClass('fa fa-check active-color')));
                ...
                $this.setColor(colors[0].Hex);
            }
            /*global color setting code ends here*/

            /*global font setting code starts here*/
            // fonts List
            //console.log("CODE ERROR: 9");
            var fontFamily = [{
            ...
            //console.log("CODE ERROR: 10");

            var globalFont = charity.global_font;
            if (globalFont == '') {
                globalFont = "'Montserrat',sans-serif";
                $this.setFamily(globalFont);
            } else {
                $this.setFamily(globalFont);
            }

        },
        /*global font setting code ends here*/
        ...
        events : function() {
            ...
            /*full-width/boxed layout code starts here*/
            var layoutColumn = charity.layout;
            var layoutObj = $('#layout .layout-column');
            layoutObj.removeClass('radio-active');

            for (var i = 0; i < layoutObj.length; i++) {

                if (layoutColumn != '') {
                    if ($(layoutObj[i]).attr('id') == layoutColumn) {
                        //$(layoutObj[i]).addClass('radio-active');
                        var layoutStyle = $(layoutObj[i]).attr('data-layout')
                        styleSwitcher.layoutStyle(layoutStyle);
                    }

                } else {
                    //$(layoutObj[0]).addClass('radio-active');
                    var layoutStyle = $(layoutObj[0]).attr('data-layout')
                    styleSwitcher.layoutStyle(layoutStyle);
                }
            ...
            /*full-width layout code ends here*/

            /*sticky header code starts here*/
            /*$(document).on('click', '#sticky-no, #sticky-yes', function() {
                var sticky = $(this).attr('rel')
                styleSwitcher.stickyHeader(sticky);
            });

            var stickyHeader = charity.sticky_header;
            var stickyHeaderObj = $('#sticky .layout-column');
            stickyHeaderObj.removeClass('radio-active');

            for (var i = 0; i < stickyHeaderObj.l */
            ...
            /*sticky header layout code ends here*/
