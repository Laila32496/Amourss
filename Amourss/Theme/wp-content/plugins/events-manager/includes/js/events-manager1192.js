jQuery(document).ready( function($){
	...
	$(document).delegate('.em-search-form, .em-events-search-form', 'submit', function(e){
    	...
    	if( results_wrapper.length > 0 ){
    		...
    		if( img.length > 0 ) img.attr('src', img.attr('src').replace('search-mag.html', 'search-loading.html'));
    		...
    		$.ajax( EM.ajaxurl, {
			    ...
			    success : function(responseText){
			    	...
			    	if( img.length > 0 ) img.attr('src', img.attr('src').replace('search-loading.html', 'search-mag.html'));
